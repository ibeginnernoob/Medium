...
function ImageBlogCard({authorName,publishDate,title,description,id,imageURL,saved,savePost,userBlog,deletePost}:props){
    ...
    return(
        <>
            <ConfirmationModal open={open} setOpen={setOpen} deletePost={deletePost} id={id} />
            <div className="mx-10 flex flex-col items-center py-6 border-b-[1px] border-gray-200 cursor-pointer md:justify-between md:mx-24 sm:flex-row">
                <div className="w-full flex flex-col md:w-[70%]">
                    <div className='flex flex-row items-center mb-3'>
                        ...
                        {!userBlog ? <p>&#183;</p> : null}
                        {!userBlog ? <p className='text-sm text-gray-400 px-1.5'>Posted on {getFormattedDate(publishDate)}</p> : null}
                        {userBlog===true ? <><button className="mx-5 text-xs text-white bg-cyan-500 border-blue-400 border-[2px] px-2 py-1 rounded-2xl hover:opacity-80" onClick={(e)=>{
                            ...
                            <button className="text-xs text-white bg-red-500 border-red-800 border-[2px] px-2 py-1 rounded-2xl hover:opacity-80" onClick={(e)=>{
                                setOpen(true)
                                e.preventDefault()
                            }}>Delete</button></> : null}
                    </div>
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <div className='flex flex-row justify-start mr-6 items-center md:mr-12 sm:bottom-[30px] sm:right-[30%] md:right-[40%] lg:right-[30%] lg:bottom-[50px]'>
                                <div className='cursor-pointer'>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill={saved ? "currentColour" : "none"} viewBox="0 0 24 24" strokeWidth={1.2} stroke="currentColor" className={`size-5 ${saved ? "hover:opacity-60" : "hover:opacity-30"}`} onClick={async (e)=>{
                                        savePost(id)
                                        e.preventDefault()
                                    }}>
                                        {!saved ? 
